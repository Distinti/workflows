...
> content is restricted to files named `CLAUDE.md` (this file and nested ones).
>
> Agents working in this repo MUST:
>
> - Treat all non-`CLAUDE.md` files as canonical human work
> - **Never modify** `README.md` or any other human doc without explicit human
>   instruction
> - Defer to human docs over `CLAUDE.md` on conflict — `CLAUDE.md` is a _map_,
...
This `CLAUDE.md` is checked by the meta-repo's `scripts/lint_claude_md.py`,
which verifies every backtick-quoted local filename in a `CLAUDE.md` actually
